...
firebase = firebase.FirebaseApplication("https://py-vb-chatapp-default-rtdb.firebaseio.com", None)
login_window = Tk()
...
def enterRoom():
    ...
    friends_code = friends_code_entry.get()
    ...
    btn_send = Button(message_window, text="Send", font = 'arial 13', bg="#D6CA98", fg="black", padx=10,command=sendData() ,relief=FLAT)
...
def getData():
    ...
    get_friends_data = firebase.get('/', friends_code)
    
    if(get_friends_data != None):
        print("friends data " +get_friends_data)
